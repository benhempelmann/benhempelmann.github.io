...
void main() {
  ...
  float u = (gl_FragCoord.x - u_origin.x) / (u_scale.x * u_origin.x) + u_center.x;
  float v = (u_resolution.z - gl_FragCoord.y - u_origin.z) / (u_scale.x * u_origin.z) + u_center.y;
  vec2 uv = vec2(u,v);  

  vec4 dFrag = vec4(ds_set(gl_FragCoord.x), ds_set(gl_FragCoord.y));
